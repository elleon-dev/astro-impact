...
const ImpactAnimation = ({ simData }: { simData: SimulationData }) => {
  ...
  useFrame((state) => {
    ...
    if (explosionRef.current && time > 5) {
      ...
      explosionRef.current.scale.set(
        explosionScale,
        explosionScale,
        explosionScale,
      );
  ...
  return (
    <>
      ...
      <Sphere ref={earthRef} args={[3, 64, 64]} position={[0, 0, 0]}>
        <meshStandardMaterial color="#2563eb" roughness={0.7} metalness={0.2} />
      </Sphere>
      ...
      <Sphere ref={explosionRef} args={[0.5, 32, 32]} position={[0, 0, 0]}>
        <meshBasicMaterial color="#ff6b35" transparent opacity={1} />
      </Sphere>
    </>
  );
};
...
export const ImpactVideo = ({ simData }: ImpactVideoProps) => {
  return (
    <div className="w-full h-[calc(100vh-13rem)] bg-gradient-to-b from-background via-background/90 to-background relative">
      ...
      {/* Overlay Text */}
      <div className="absolute top-18 left-8 bg-background/80 backdrop-blur-sm p-6 rounded-lg border border-border max-w-md">
