...
const AsteroidApproach = ({ simData, onComplete }: { simData: SimulationData; onComplete?: () => void }) => {
  ...
  useEffect(() => {
    ...
    const material = new THREE.LineBasicMaterial({ color: '#f25c05', transparent: true, opacity: 0.3 });
  ...
  useFrame((state) => {
    ...
    if (asteroidRef.current && time < duration) {
      ...
      if (trajectoryRef.current) {
        const material = trajectoryRef.current.material as THREE.LineBasicMaterial;
  ...
  return (
    <div className="w-full h-screen bg-gradient-to-b from-black via-secondary/20 to-black relative">
      <Canvas camera={{ position: [0, 8, 18], fov: 60 }}>
        ...
        <Stars radius={300} depth={60} count={10000} factor={8} fade speed={1} />
      ...
      <div className="absolute top-24 left-8 bg-gradient-to-br from-black/60 to-transparent backdrop-blur-sm p-6 rounded-xl max-w-md">
        <h2 className="text-2xl font-bold mb-3 text-white">Trayectoria de Aproximación</h2>
        <p className="text-white/80 text-sm leading-relaxed">
          Asteroide tipo <span className="text-primary font-semibold">{simData.asteroidType}</span> de{" "}
          <span className="text-primary font-semibold">{simData.diameter}m</span> aproximándose a{" "}
          <span className="text-primary font-semibold">{simData.velocity} km/s</span>.
        </p>
        <p className="text-white/60 text-xs mt-3">
          Puedes rotar la vista con el mouse para observar la trayectoria desde diferentes ángulos.
        </p>
      </div>
    </div>
  );
